...
```bash
aws s3api list-objects-v2 --bucket <S3_BUCKET_NAME> --prefix <PREFIX_PATH>/ --profile <AWS_PROFILE> --query 'Contents[].Key' --output text | tr '\t' '\n' | grep -E 'file_pattern' > <OUTPUT_KEYS_FILE>.txt
```

Use `--s3-keys-file` for the text-file workflow or `--manifest` for a CSV/TSV manifest. For example:

```bash
python scripts/bulk_import.py \
  --project username/my-project \
  --volume username/my-volume \
  --manifest manifests/import_manifest.tsv \
  --profile cavatica \
  --run
```
...
## Import Files from a Manifest (Deprecated)

`scripts/deprecated/import_from_manifest.sh` is the legacy manifest-import workflow. It is deprecated because manifests generated by the data-modeling repository have been updated; use `bulk_import.py` with `--manifest` for the current workflow. The legacy script remains available as a starting point for other use cases.

The legacy script searches an AWS S3 bucket for keys matching the pattern in column 6 of the reviewed manifest, excludes `.md5` files, and passes the matching keys to `bulk_import.py` for submission. It only supports AWS-backed volumes and requires the source bucket and Cavatica volume to be configured.
...
```bash
bash scripts/deprecated/import_from_manifest.sh \
  -a <aws_bucket_name> \
  -c <cavatica_profile> \
  -m <manifest_file> \
  -p <project_id> \
